...
export default function RegisterPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const msg = err?.data?.message || (err?.data?.errors ? Object.values(err.data.errors).flat().join(" ") : null) || err?.message || "Registration failed";
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-wood-100">
      ...
      <main className="mx-auto max-w-md px-4 py-12">
        <h1 className="mb-6 text-2xl font-bold text-wood-900">Sign up</h1>
        {error && (
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <label className="flex flex-col gap-1">
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="rounded-lg border border-wood-500/40 bg-white/10 px-4 py-2 text-wood-900 outline-none focus:ring-2 focus:ring-accent focus:border-accent" />
          </label>
          <label className="flex flex-col gap-1">
            <span className="text-sm font-medium text-wood-700">Email</span>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="rounded-lg border border-wood-500/40 bg-white/10 px-4 py-2 text-wood-900 outline-none focus:ring-2 focus:ring-accent focus:border-accent" />
          </label>
          <label className="flex flex-col gap-1">
            <span className="text-sm font-medium text-wood-700">Phone (optional)</span>
            <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className="rounded-lg border border-wood-500/40 bg-white/10 px-4 py-2 text-wood-900 outline-none focus:ring-2 focus:ring-accent focus:border-accent" />
          </label>
          <label className="flex flex-col gap-1">
            <span className="text-sm font-medium text-wood-700">Password</span>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="rounded-lg border border-wood-500/40 bg-white/10 px-4 py-2 text-wood-900 outline-none focus:ring-2 focus:ring-accent focus:border-accent" />
          </label>
          <label className="flex flex-col gap-1">
        ...
        <p className="mt-4 text-center text-sm text-wood-600">
          Already have an account? <Link href="/login" className="font-medium text-wood-900 underline hover:text-accent">Log in</Link>
        </p>
      </main>
    </div>
  );
